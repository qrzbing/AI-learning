...
# 写入记录函数
# tfRecordName: 存放 tfRecords 文件的路径和文件名
# image_Path: 图像路径
# label_Path: 标签文件路径
...
def generate_tfRecord():
    # 判断保存路径是否存在
    print("[?] Judge if record exists")
    isExists = os.path.exists(data_path) 
    if not isExists: 
        # 创建文件夹
        print('[-] Record not exists')
        os.makedirs(data_path)
        print("[+] The directory was created succesfully")
    else:
        print("[+] Directory already exists")
    # 用下述自定义函数将训练集中的图片和标签生成
    # 名叫 tfRecord_train 的 tfRecord 文件 
    write_tfRecord(
        tfRecord_train,
        image_train_path,
        label_train_path
